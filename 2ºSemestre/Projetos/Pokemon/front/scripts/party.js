...
const choseMoves = (moves) => {
    ...
    let moveSet = document.querySelector(".moves")
    
    rngMove.forEach((move, index) => {
        ...
        .then(desc => {
            ...
            moveSet.querySelector(`.${classe}`).innerHTML = (desc.name).toUpperCase();
            moveSet.querySelector(`.${classe}`).addEventListener('click', () => {useMove(index)});
            combat.party.push(desc)
            //console.log(desc.id);
        })
    })
}
